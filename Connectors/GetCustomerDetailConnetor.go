...
func GetBookDetail(c *gin.Context, dataMap map[string]any) (err error) {
	//var reqDetail Models.Product
	var resDetail Models.Product
	...
	var apiUrl = "http://localhost:8181/api/GetBukuByJudulOrcl"
	client := http.Client{}

	req, err := http.NewRequest("GET", apiUrl, reqBody)
	fmt.Println(err)
	fmt.Println(req)
	if err != nil {
		return
